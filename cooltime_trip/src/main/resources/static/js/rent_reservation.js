...
/* $('#rentReservationBtn').on(submit,function(){
...
$('#rentReservationBtn').click(function(){
	if($('#driverKrName').val() == ''){
		alert("이름을 입력해주세요");
    	return false;
	}else if($('#driverEmail').val() == '' || $('#driverEmailDomain').val() == ''){
		alert("이메일을 입력해주세요");
    	return false;
	}else if($('#driverPhoneNum1').val() == '' || $('#driverPhoneNum2').val() == ''){
		alert("휴대폰 번호를 입력해주세요");
    	return false;
	}else if($('#driverBirth').val() == ''){
		alert("생년월일을 입력해주세요");
    	return false;
	}
	...
	if(!(pattern_kor1.test(driverKrName.value)) || pattern_num.test(driverKrName.value) || pattern_eng.test(driverKrName.value) || pattern_spc.test(driverKrName.value)){
  	...
  	if(!($('.check_agree_terms>i').hasClass('fa-active'))){
  		alert("약관에 동의해주세요");
    	return false;
  	}
